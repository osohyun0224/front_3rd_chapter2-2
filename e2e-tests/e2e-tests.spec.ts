...
test('사용자가 장바구니에 상품을 추가하고 쿠폰을 적용하여 최종 결제 금액을 확인할 수 있어야 한다.', async ({ page }) => {
  await page.goto('http://localhost:5173/index.refactoring.html');
  await expect(page).toHaveURL('http://localhost:5173/index.refactoring.html');
...
test('사용자가 장바구니에서 상품 수량을 조절하고 상품을 삭제하여 최종 결제 금액이 정확하게 반영되는지 확인한다.', async ({ page }) => {
  await page.goto('http://localhost:5173/index.refactoring.html');
  ...
  await expect(page.locator('text=최종 결제 금액: 30,000원')).toHaveText('최종 결제 금액: 30,000원');

  const minusButton = page.locator('button', { hasText: '-' });
  await minusButton.click();

  await expect(page.locator('text=최종 결제 금액: 20,000원')).toHaveText('최종 결제 금액: 20,000원');
...
test('관리자가 새로운 상품을 추가하면 상품 목록에 해당 상품이 정상적으로 표시되어야 한다.', async ({ page }) => {
  await page.goto('http://localhost:5173/index.refactoring.html');
...
test('관리자가 새 쿠폰을 등록하면 쿠폰 목록에 쿠폰이 정상적으로 표시되어야 한다.', async ({ page }) => {
  await page.goto('http://localhost:5173/index.refactoring.html');
  await page.locator('button', { hasText: '관리자 페이지로' }).click();
  ...
  await expect(page.locator('data-testid=coupon-3')).toHaveText('1000원 할인 쿠폰 (AMOUNT1000):1000원 할인');
});

test('관리자가 등록한 새 쿠폰을 장바구니 페이지에서 적용할 때 쿠폰에 따른 할인이 정확하게 반영되어야 한다.', async ({ page }) => {
  await page.goto('http://localhost:5173/index.refactoring.html');
  await page.locator('button', { hasText: '관리자 페이지로' }).click();
