...
export const CartDetail: FC<CardDetailProps> = ({ item, onClickUpdateQuantity, onClickRemoveCart }) => {
    ...
    <div key={item.product.id} className="flex justify-between items-center bg-white p-3 rounded shadow">
      ...
      <div>
        ...
        <Button
          ...
          text="+"
        />
        <Button size="small" color="error" onClick={() => onClickRemoveCart(item.product.id)} text="삭제" />
      </div>
    </div>
  )
}
